...
export namespace AllocationService {
  ...
  export async function publishAllocation(allocationId: string) {
    for (let attempt = 0; attempt < MAX_PUBLIC_ID_RETRIES; attempt++) {
      ...
      try {
        ...
        throw new AllocationServiceServer("Unknown error", "UNKNOWN_ERROR", error);
      }
    }
  ...
  export async function zipAllocationWithUsFiscalBudget(
    ...
    invariant(allocatableBudgetBpsTotal > 0, "Allocatable budget total must be greater than zero");

    const allocatableRows = allocatableFunctions.map((f) => {
      const participantAllocation = allocation.items.find((a) => a.categoryCode === f.id) || {
        weightBps: 0,
      };
...
export type AllocationServiceErrorCodes = "UNKNOWN_ERROR" | "PUBLIC_ID_COLLISION";

export class AllocationServiceServer extends Error {
  public readonly code: AllocationServiceErrorCodes;
  public readonly originalError?: Error | unknown;

  constructor(message: string, code: AllocationServiceErrorCodes, originalError?: Error | unknown) {
    super(message);
    this.code = code;
