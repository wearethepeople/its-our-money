...
import { type ToastInput, toastSessionStorage, toastKey } from "@/utils/toast.server.ts";
...
expect.extend({
  ...
  async toHaveSessionForUser(response: Response, participantId: string) {
    ...
    const authSession = await sessionStorage.getSession(convertSetCookieToCookie(sessionSetCookie));
    ...
    return {
      ...
      message: () =>
        `A session was${this.isNot ? " not" : ""} created in the database for ${participantId}`,
    };
  },
